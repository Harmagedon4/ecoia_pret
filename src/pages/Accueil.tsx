  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const totalDue = loans.reduce((sum: number, loan: any) => sum + loan.amount, 0);
        const activeLoans = loans.filter((loan: any) => loan.status === 'pending' || loan.status === 'approved').length;
        const monthlyPayments = Math.round(loans.reduce((sum: number, loan: any) => sum + (loan.amount * indicativeRate / 12), 0));
        const nextPayment = loans.length > 0 ? loans[0].createdAt : '';

        setStats({ totalDue, activeLoans, monthlyPayments, nextPayment });
        setRecentLoans(loans.slice(0, 5));
        setUpcomingPayments(loans.map((loan: any) => ({
          date: loan.createdAt,
          amount: Math.round(loan.amount * indicativeRate / 12),
          type: loan.title
        })));
      } catch (err) {
        console.error(err);
    ...
    <div className="space-y-6 p-6">
      ...
      <Card>
        ...
        <CardContent className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          <Button asChild variant="outline" className="h-auto flex flex-col items-center p-4 space-y-2 rounded-lg border border-gray-200 hover:bg-gray-100 transition-colors">
          ...
          <Dialog>
            <DialogTrigger asChild>
              <Button variant="outline" className="h-auto flex flex-col items-center p-4 space-y-2 rounded-lg border border-gray-200 hover:bg-gray-100 transition-colors">
            ...
            <DialogContent className="sm:max-w-md p-6 bg-white rounded-lg shadow-lg">
              <DialogHeader>
                <DialogTitle className="text-xl font-semibold text-gray-900">Simulateur de prêt</DialogTitle>
              ...
              <div className="space-y-6">
                <div>
                  <Label htmlFor="amount" className="block text-sm font-medium text-gray-700 mb-1">Montant demandé *</Label>
                  <div className="flex items-center space-x-2">
                    <Input
                      ...
                      step={10000}
                    />
                    <span className="px-3 py-2 bg-gray-100 text-gray-600 rounded-md text-sm">XOF</span>
                  </div>
                </div>

                <div>
                  <Label htmlFor="duration" className="block text-sm font-medium text-gray-700 mb-1">Durée du prêt *</Label>
                  <div className="flex items-center space-x-2">
                    <Input
                      id="duration"
                      type="number"
                      ...
                      max={300}
                    />
                    <span className="px-3 py-2 bg-gray-100 text-gray-600 rounded-md text-sm">mois</span>
                  </div>
                </div>

                <div>
                  <Label htmlFor="purpose" className="block text-sm font-medium text-gray-700 mb-1">Objet du prêt *</Label>
                ...
                <div>
                  <Label className="block text-sm font-medium text-gray-700 mb-1">Type de taux préféré *</Label>
                  <RadioGroup value={rateType} onValueChange={setRateType} className="flex space-x-4">
                    <div className="flex items-center space-x-2">
                      <RadioGroupItem value="fixe" id="fixe" />
                      <Label htmlFor="fixe" className="text-sm text-gray-700">Taux fixe</Label>
                    </div>
                    <div className="flex items-center space-x-2">
                      <RadioGroupItem value="variable" id="variable" />
                      <Label htmlFor="variable" className="text-sm text-gray-700">Taux variable</Label>
                    </div>
                  </RadioGroup>
                </div>
                ...
                {error ? (
                  ...
                  <div className="p-4 bg-green-50 rounded-md">
                    <p className="text-sm font-semibold text-green-800">Mensualité estimée :</p>
                    <p className="text-lg font-bold text-green-600">{monthlyPayment}/mois</p>
              ...
              <DialogClose asChild>
                <Button variant="outline" className="w-full mt-4 px-4 py-2 border border-gray-300 rounded-md text-gray-700 hover:bg-gray-100">
                  Fermer
                </Button>
              </DialogClose>
            </DialogContent>
            ...
            {recentLoans.map((loan) => (
              <div key={loan.id} className="flex items-center justify-between p-4 border rounded-lg">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {loan.status === 'approved' && <span className="text-sm text-muted-foreground">{loan.progress || 0}%</span>}
                </div>
              </div>
            ))}
          </div>
      ...
      <Dialog open={showScheduleModal} onOpenChange={setShowScheduleModal}>
        ...
        <DialogContent className="sm:max-w-md p-6 bg-white rounded-lg shadow-lg">
          <DialogHeader>
            <DialogTitle className="text-xl font-semibold text-gray-900">Calendrier des échéances</DialogTitle>
            <DialogDescription className="text-sm text-gray-500">Consultez toutes vos échéances de paiement</DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
          ...
          <DialogClose asChild>
            <Button variant="outline" className="w-full mt-4 px-4 py-2 border border-gray-300 rounded-md text-gray-700 hover:bg-gray-100">Fermer</Button>
          </DialogClose>
        </DialogContent>
      </Dialog>
    </div>
